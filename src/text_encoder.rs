...
pub trait TextEncoder<T> {
    ...
    /// # Errors
    ///
    /// Returns [`TextEncodingErrorKind::BufferTooSmall`] when `output` cannot hold
    ...
    /// # Errors
    ///
    /// Returns [`TextEncodingErrorKind::InvalidCodePoint`] when `code_point` is not
    /// a Unicode scalar value. Returns any error reported by [`Self::encode_char`]
    /// for valid scalar values.
    fn encode_code_point(&self, code_point: u32, output: &mut [T]) -> TextEncodingResult<usize> {
        match Unicode::to_char(code_point) {
            ...
            None => Err(TextEncodingError::new(
                self.encoding(),
