...
fn test_utf32_byte_codec_exposes_encoder_and_decoder_contracts() {
    ...
    assert_eq!(
        ...
        <Utf32ByteCodec as CharsetCodec>::max_units_per_char(&codec)
    );
    assert_eq!(
        ...
        <Utf32ByteCodec as CharsetCodec>::decode_one(&codec, &[], 0)
            .expect("utf32 bytes need more"),
    );
    assert_eq!(
        4,
        <Utf32ByteCodec as CharsetCodec>::encode_one(&codec, 'A', &mut [0_u8; 4], 0)
            .expect("encode utf32 unit bytes"),
    );
